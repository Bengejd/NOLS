...
const checkForProcessArgs = async () => {
  return new Promise((resolve) => {
    const processMode = process.argv[2];
    if (!processMode) return resolve(MODE.standard = true);
    console.log(chalk.red(`nols ${processMode} enabled`));

    if (processMode.toLowerCase() === '--revert') {
      return resolve(MODE.revert = true);
    } else if (processMode.toLowerCase() === '--clean') return resolve(MODE.clean = true);
  });
};

// Show the entry logo and name of the library.
const showLogo = () => {
  console.log(chalk.green('NOLS: No One Likes Stylesheets \n Please be patient...'));
};
...
const start = async () => {
  ...
  // Read the file line by line and convert it.
  files.map((file) => readAndTranslateFile(file)
  // Write that conversion back to file.
    .then((newFile) => writeFile(file, newFile)));
};
